...
La aplicación se apoya en pocas librerías donde cada una cubre una responsabilidad concreta.
...
La arquitectura del proyecto consiste en la siguiente arquitectura por capas:
...
De esta forma se consigue que los calculos esten aislados en clases de Java puras.

## 4. MainActivity, navegación común y llamada a Gemini

`MainActivity` consiste en realizar las siguientes tres operaciones:
...
De esta forma cualquier pestaña que implemente esa interfaz puede decirle a `MainActivity` como debe llamarse en pantalla y qué datos necesita gemini para analizarla.
...
`UiUtils.formatLlmResponse(...)` limpia el texto generado por el modelo para que se vea bien en Android.
...
En la pestaña `Micro` el usuario escribe un ticker, la app descarga el histórico diario y enseguida construye dos lecturas:
...
El archivo se llama `FpcaEngine`, el código hace un PCA clásico sobre una discretización finita de la curva.
...
En lugar de mirar series aisladas: 

1. se enseña la curva actual como una forma,
...
La estrategia `OnConflictStrategy.REPLACE` consiste en ver si el usuario vuelve a meter un ticker existente, la fila se sobrescribe con el número actualizado de acciones.
...
La app no usa pesos arbitrarios escritos a mano. Los deduce a partir de las posiciones y del último precio disponible.
...
El CVaR se calcula de forma empírica sobre la cola histórica.
...
Ese resultado se multiplica por 100 para mostrarlo como porcentaje. Es un **percentil de volatilidad histórica realizada**.
...
De esta forma se junta ruido diario, volatilidad media y liquidez en un solo panel.
...
En este apartado se explicará resumidamente cada pantalla por separado.
...
En casi toda la app se repite la misma estructura:
...
> Imagen: captura de la biblioteca con búsqueda y varios términos.
...
### 10.3. La matemática está desacoplada del framework Android
...
### 10.4. La IA no inventa los datos de la pantalla
...
`QuantTrack` combina varias capas de trabajo como:
...
La aplicación tiene como idea principal tomar series de mercado, resumirlas con herramientas cuantitativas conocidas y convertirlas en una interfaz que cualquiera del equipo pueda leer sin perderse.
